...
export const getAll = async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(
      `SELECT category_id, name, description, image_url, menu_type_id
       FROM categories`
    );
...
export const getCategoriesWithProducts = async (req: Request, res: Response) => {
  try {
    ...
    rows.forEach((row: { category_id: string | number; category_name: any; category_description: any; category_image: any; menu_type_id: any; product_id: any; product_name: any; description: any; price: any; product_image: any; }) => {
...
export const getByMenuType = async (req: Request, res: Response) => {
  try {
    const menuTypeId = Number(req.params.type);

    // ✅ numeric validation
    if (!Number.isInteger(menuTypeId) || ![1, 2].includes(menuTypeId)) {
    ...
    console.error('Error fetching categories by menu type:', err);
    res.status(500).json({ message: 'Server error' });
  }
};
